...
const Signup = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!formData.password || ! formData.cpassword || !formData.firstName || !formData.lastName || !formData.email || !formData.phoneNumber) {
            toast.error("Fill all required feilds");
            return;
        }
        ...
        try {
            const response = await fetch(`${apiUri}api/CreateUser`, {
                method: 'POST',
                headers: {
                    ...
                    <div className="card-body ">
                        ...
                        <div className="row justify-content-center">
                            <div className=" col-md-6">
                                ...
                                <button className={`btn btn-outline-info mt-3  p-3 fw-bold  fs-4  me-2 text-${isDarkMode? "light": "dark"}`} onClick={()=> toast("Oops... Try Siginig up with Credentials")}>Sign up with <i className="fa-brands fa-google  text-danger fs-1 ms-2 "></i>  </button>
                            </div>
                                </div>
                            </div>
                        </div>
